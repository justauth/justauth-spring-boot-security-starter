...
```
### 5. 访问前端页面
- 浏览器访问 `http://localhost:9090/demo/login.html`, 至此集成了：第三方登录(JustAuth); 实现快速开发。
...
### 必须添加 top.dcenter.ums.security.core.oauth.config.Auth2AutoConfigurer 到 HttpSecurity    
```java
@Configuration
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
...
| ums.oauth.redirectUrlPrefix                                  | String         | /oauth/login         | 第三方登录回调处理 url 前缀 ，也就是 `RedirectUrl` 的前缀, 不包含 `ServletContextPahth`，默认为 `/oauth/login` |                       |
...
| ums.oauth.remainingExpireIn                                  | Integer        | 24                   | `accessToken` 的剩余有效期, 默认: 24, 单位: 小时. 注意: 需要根据实际生产环境进行优化 |                       |
| **justAuth**                                                 |                |                      |                                                              |                       |
